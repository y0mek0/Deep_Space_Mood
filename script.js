...
function getUniformsForTheme(theme) {
    ...
    if (theme === 'gargantua') return { brightness: { value: 0.00073 }, darkmatter: { value: 0.1 }, distfading: { value: 0.73 }, saturation: 0.95, formuparam: 0.533, zoom: 0.997, tile: 4.585, speed: -0.0001 };
    return common;
}
...
function getSystemPromptForTheme(theme) {
    ...
    if (theme === 'backbone') return `You output ONLY valid JSON for a complex abstract shader. ${artisticRule} The JSON must follow this structure: { "core_color": "#ff8c00", "abstraction": float (0.6 to 0.8), "gamma": float (1.0 to 1.2), "brightness": float (1.0 to 1.5), "nebula_strength": float (0.9 to 1.0), "movement_speed": float (0.1 to 1.0) }`;
